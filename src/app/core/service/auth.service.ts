...
export class AuthService {
      ...
      .pipe(
        map((data: any) => {
          if (
            ...
            data.meta.statusCode === API.AUTHENTICATE.STATUS.BAD_CREDENTIAL
          ) {
      ...
      .pipe(
        map((data: any) => {
          if (
            ...
            data.meta.statusCode === API.AUTHENTICATE.STATUS.ACCOUNT_EXISTED
          ) {
            return 'Account existed';
          } else if (
            data.meta.statusCode === API.AUTHENTICATE.STATUS.ACCOUNT_INACTIVE
          ) {
            return 'Account was not activated';
          } else if (
            data.meta.statusCode === API.AUTHENTICATE.STATUS.ACCOUNT_LOCKED
          ) {
  ...
  roleMatch(allowedRoles: any): boolean {
    const userRoles: any =
      this.storageService.getItemLocal('userInfo').userRoles[0].roleName;
